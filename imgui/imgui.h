...
struct ImVec2
{
    ImVec2 operator* (float number)
    {
        return ImVec2(x * number, y * number);
    }
    float                                   x, y;
...
struct ImVec4
{
    ImVec4 operator* (float number)
    {
        return ImVec4(x * number, y * number, z * number, w * number);
    }
    float                                                     x, y, z, w;
...
namespace ImGui
{
    ...
    IMGUI_API bool          InputText(const char* label, char* buf, size_t buf_size, const bool renderText = true, ImGuiInputTextFlags flags = 0, ImGuiInputTextCallback callback = NULL, void* user_data = NULL);
